  ...
  ```bash
     ...
     - 고민했던 내용
       ...
       > idea 1. ":" in num ?
         ...
         * "숫자:숫자", "숫자:문자", "문자:숫자", "문자:문자" 등
           입력받은 문자열 안에 ":"이 있기만 하면 True -> X
       > idea 2. 정규표현식 ?
         * 정규표현식을 사용하여 입력받은 문자열이 패턴과 일치하는지 확인
         * \d+:\d+ -> \d(숫자 0~9)가 +(1개 이상) 있고, 그 사이에 ":"이 있는 형태 -> O
         * re.match : 시작 문자열에서만 확인
         * => '1:1 창세기'가 입력된 경우, 시작 문자열에 해당 패턴이 존재하므로 True -> X
         * re.search : 전체 문자열에서 확인
         * => '창세기 1:1'을 입력받은 경우, 문자열 내에 해당 패턴이 존재하므로 True -> X
  
     - 해결 방법
       > re.fullmatch(r"\d+:\d+", num)
         * re.fullmatch : 문자열 전체가 패턴과 일치
         * 입력받은 문자열 전체가 패턴과 정확하게 - 일치하면 True
         *                                  - 일치하지 않으면 False
         * '숫자:숫자' 형식만 입력받을 수 있으며, 형식에 맞지 않는 경우 '올바르지 않은 입력' 메세지 출력 -> O
  ```
